...
public final class ConfigManager {
    ...
    public static void initialize(final ModContainer modContainer) {
        CONFIGS.forEach((spec, config) -> {
            ...
            modContainer.registerConfig(platformType, spec);
        });

        modContainer.getEventBus().addListener(ConfigManager::handleModConfigEvent);
    }
    ...
    private static <T> void fillSpec(final T instance, final Builder builder, final ArrayList<ConfigFieldPair<?>> values) {
        for (final Field field : instance.getClass().getFields()) {
            parseField(instance, builder, values, field);
        }
    }

    private static ConfigDefinition createDefinition(Object instance, ArrayList<ConfigFieldPair<?>> values) {
    ...
    private static String getPath(final Field field) {
        final li.cil.scannable.util.config.Path pathAnnotation = field.getAnnotation(li.cil.scannable.util.config.Path.class);
    ...
    private interface ConfigValue<T> {
        T get();
    }

    // --------------------------------------------------------------------- //
    // NeoForge Builder/ConfigValue implementations
